...
model.load_state_dict(torch.load('./data/model/Model_abs.pth', map_location=device))
model.eval()
...
class GraphFingerprintsModel(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size,
        ...
        total_input_dim = graph_feat_size + graph_feat_size + 2 * graph_feat_size
        # Final concatenation for prediction (3 * graph_feat_size)
        self.predict = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(total_input_dim, 128),
            nn.ReLU(),
...
# Load saved model parameters
model.load_state_dict(torch.load('./data/model/Model_em.pth', map_location=device))
model.eval()
...
# Load saved model parameters
model.load_state_dict(torch.load('./data/model/Model_plqy.pth', map_location=device))
model.eval()
...
# === Label standardization (only for interface consistency, not needed for prediction) ===
target_data[['k']] = scaler.transform(target_data[['k']])  # Note: only for dataset construction, does not affect prediction results
...
# Load saved model parameters
model.load_state_dict(torch.load('./data/model/Model_k.pth', map_location=device))
model.eval()
